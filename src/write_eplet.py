...
def write_stronger_eplet_on_link(svg, path_position, stronger_eplet_on_link, text_size, bead_text_position):
    print(stronger_eplet_on_link)
    global_written = {}
    already_write = {}
    warning = False

    stronger_eplet_on_link, restreint = restrain_red_eplet(stronger_eplet_on_link)
    if restreint:
        if not warning:
    ...
    for couple, eplets in stronger_eplet_on_link.items():
        for eplet in eplets:
            if already_write[couple] > 2:
                ...
                if to_close_to_write(global_written, "...", x, y, bead_text_position):
                    if not warning:
                        view_box = get_view_box(svg)
                        svg = write_warning_message(view_box, svg)
                        warning = True
...
def write_strong_eplet_on_link(svg, path_position, strong_eplet_on_link, text_size, stronger_eplet_on_link,
                               bead_text_position, global_written_stronger_link):
...
def write_strong_eplet_on_bead(svg, bead_position, strong_eplet_on_bead, stronger_eplet_on_bead, strong_eplet_on_link,
...
def write_warning_message(view_box, svg):
    ...
    svg = write_svg.write_text_on_svg(
        ...
        font_family="Dialog",
        color="#ba4a00",
        text="Please check all positives eplets."
    )
    return svg

def restrain_red_eplet(eplet_dico):
    new_dict = {}
    warning = False

    for pair, eplets in eplet_dico.items():
        if len(eplets) > 2:
            new_dict[pair] = eplets[:2]
            warning = True
        else:
