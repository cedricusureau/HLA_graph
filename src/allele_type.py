...
def write_svg_for_allele(template_graph, mfi,edges,eplet, output,allele_type,cutoff):
    ...
    node_to_color = write_svg.node_color_to_change(circle_ligne, data, cutoff)
    ...
    pos_bead_eplet, neg_bead_eplet = eplet_management.get_eplet_from_beads(data, eplet, allele_type,cutoff)
    ...
    second_class_eplet = eplet_management.get_second_class_eplet(pos_eplet_ratio_dict, neg_eplet_ratio_dict)

    # Vérifie les liens concerné par un partage d'éplet présent sur certaines billes positives mais pas les billes négatives
    link_for_second_class_eplets = eplet_management.get_link_for_second_class_eplets(link_between_pos, eplet,
                                                                          second_class_eplet)

    # Ecrit en bleu les noms des eplets concerné
    new_svg2 = write_svg.write_class_2_eplet(new_svg, link_for_second_class_eplets,
                                            middle_position_between_positive_beads)

    #Extrait tout les eplets déjà marqué
    eplets_on_isolated_beads = eplet_management.get_eplets_on_isolated_beads(ratio, second_class_eplet)
    ...
    new_svg3 = write_svg.write_3_class_eplet(new_svg2, eplets_on_isolated_beads, isolated_bead,
                                            position_of_isolated_beads, eplet)
